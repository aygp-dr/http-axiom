...
# --------------------------------------------------------------------------
#
# Models output square (1024x1024). Non-square formats derived via
# imagemagick. File-based targets: only regenerate when source changes.
#
# Workflow:
#   make images/output/01-pipeline-hero_banner.png
#   make images-all
...
# Source images (generated by ollama from prompt files)
PROMPT_NAMES := $(basename $(notdir $(wildcard $(IMGPROMPTS)/*.txt)))
HERO_SRC     := $(IMGOUT)/01-pipeline-hero_$(subst /,_,$(subst :,_,$(OLLAMA_MODEL)))_s$(OLLAMA_SEED).png

# Derived sizes from hero source
...
# Generate source images from prompts via ollama
$(IMGOUT)/%.png: $(IMGPROMPTS)/%.txt
	@mkdir -p $(IMGOUT)
	python3 images/generate.py --model $(OLLAMA_MODEL) --seed $(OLLAMA_SEED) --index $$(echo $* | cut -d- -f1 | sed 's/^0*//')

images: ## Generate all source images via ollama
...
images-all: images ## Generate sources + all derived sizes
	...
	@for src in $(IMGOUT)/*_s$(OLLAMA_SEED).png; do \
		...
		$(MAKE) "$(IMGOUT)/$${base}_banner.png" \
			"$(IMGOUT)/$${base}_og.png" \
			"$(IMGOUT)/$${base}_twitter.png" \
			"$(IMGOUT)/$${base}_thumb.png" 2>/dev/null; \
	done
